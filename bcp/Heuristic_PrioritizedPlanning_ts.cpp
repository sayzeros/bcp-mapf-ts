...
#define HEUR_NAME             "mapfts-pp"
#define HEUR_DESC             "MAPF-TS prioritized planning"
                ...
                const int time_spacing)
{
    ...
    // Block vertices in future paths.
    {
        ...
        {
          path_n = path[t].n;
          for (int h = -time_spacing; h <= time_spacing; ++h)
          {
            const auto prev_time = t + h - 1;
            if (prev_time >= 0)
            {
              {
            ...
            {
              {
                  auto& penalties = global_edge_penalties.get_edge_penalties(path_n, 0);
                  penalties.north = std::numeric_limits<Cost>::infinity();
              }
              {
                  auto& penalties = global_edge_penalties.get_edge_penalties(path_n, 0);
...
SCIP_DECL_HEUREXEC(heurExecPrioritizedPlanning)
{
    ...
    *result = SCIP_DIDNOTFIND;
    ...
    const auto time_spacing = SCIPprobdataGetTimeSpacing(probdata);
    ...
    // Get constraints for dual variable values.
    ...
    auto agent_order = Vector<std::tuple<int, int>>(); // id, |o - d|
    for (int agent_id = 0; agent_id < N; agent_id++)
    {
      agent_order.push_back(std::make_tuple(agent_id, std::abs(agents[agent_id].start_x - agents[agent_id].goal_x) + std::abs(agents[agent_id].start_y - agents[agent_id].goal_y)));
    }
    std::sort(agent_order.begin(), agent_order.end(), [](const std::tuple<int, int> &first, const std::tuple<int, int> &second)
              { return std::get<1>(first) < std::get<1>(second); }); // <: 오름차순
    for (Agent a = 0; a < N; ++a)
    {
      order.push_back(std::get<0>(agent_order[a]));
      // println("Agent: {} {}", a, std::get<0>(agent_order[a]));
    }

    // setup global_edge_penalties TODO:
    ...
    for (const auto a : order)
    {
        ...
        {
          if (aa == a) continue;
          auto path_n = agents[aa].start;
          for (int prev_time = 0; prev_time < time_spacing; ++prev_time)
          {    
            {
              const auto n = map.get_south(path_n);
        ...
        for (const auto& [nta, old_time_spacing_conflict] : old_time_spacing_conss)
        {
            ...
            if (SCIPisFeasLT(scip, dual, 0.0))
            {
                ...
                {
                  for (Time t = nta.t - 1; t <= nta.t - 1 + time_spacing; t++)
                  {
                    if (t == -1)
                    {
                      {
                          auto& penalties = edge_penalties.get_edge_penalties(nta.n, t+1);
                          penalties.north -= dual / ((double) time_spacing + 1);
                      }
                      {
                          auto& penalties = edge_penalties.get_edge_penalties(nta.n, t+1);
                          penalties.south -= dual / ((double) time_spacing + 1);
                      }
                      {
                          auto& penalties = edge_penalties.get_edge_penalties(nta.n, t+1);
                          penalties.east -= dual / ((double) time_spacing + 1);
                      }
                      {
                          auto& penalties = edge_penalties.get_edge_penalties(nta.n, t+1);
                          penalties.west -= dual / ((double) time_spacing + 1);
                      }
                      {
                          auto& penalties = edge_penalties.get_edge_penalties(nta.n, t+1);
                          penalties.wait -= dual / ((double) time_spacing + 1);
                      }
                    }
                    else 
                    {
